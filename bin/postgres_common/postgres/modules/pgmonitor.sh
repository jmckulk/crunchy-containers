...
then
    ...
    then
        ...
        then
            function_file="${CRUNCHY_DIR}/bin/postgres/modules/pgexporter/setup_pg95.sql"
        elif (( ${VERSION?} >= 90600 )) && (( ${VERSION?} < 100000 ))
        then
            function_file="${CRUNCHY_DIR}/bin/postgres/modules/pgexporter/setup_pg96.sql"
        elif (( ${VERSION?} >= 100000 )) && (( ${VERSION?} < 110000 ))
        then
            function_file="${CRUNCHY_DIR}/bin/postgres/modules/pgexporter/setup_pg10.sql"
        elif (( ${VERSION?} >= 110000 )) && (( ${VERSION?} < 120000 ))
        then
            function_file="${CRUNCHY_DIR}/bin/postgres/modules/pgexporter/setup_pg11.sql"
       elif (( ${VERSION?} >= 120000 )) && (( ${VERSION?} < 130000 ))
        then
            function_file="${CRUNCHY_DIR}/bin/postgres/modules/pgexporter/setup_pg12.sql"
        elif (( ${VERSION?} >= 130000 ))
        then
            function_file="${CRUNCHY_DIR}/bin/postgres/modules/pgexporter/setup_pg13.sql"
        else
        ...
        sed -i "s/\/usr\/bin\/pgbackrest-info.sh/\/opt\/crunchy\/postgres\/bin\/pgbackrest_info.sh/g" "/tmp/setup_pg.sql"
