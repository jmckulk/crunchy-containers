...
case $MODE in 
    postgres)
      ...
      /bin/bash $CRUNCHY_DIR/bin/postgres/start.sh
      ;;
    pgdump)
      echo_info "Starting in 'pgdump' mode"
      /bin/bash $CRUNCHY_DIR/bin/pgdump/start.sh
      ;;
    pgrestore)
      echo_info "Starting in 'pgrestore' mode"
      /bin/bash $CRUNCHY_DIR/bin/pgrestore/start.sh
      ;;
    pgbench)
      echo_info "Starting in 'pgbench' mode"
      /bin/bash $CRUNCHY_DIR/bin/pgbench/start.sh
      ;;
    pgbasebackup-restore)
      echo_info "Starting in 'basebackup-restore' mode"
      /bin/bash $CRUNCHY_DIR/bin/pgbasebackup_restore/start.sh
      ;;
    backup)
      echo_info "Starting in 'backup' mode"
      /bin/bash $CRUNCHY_DIR/bin/backup/start-backupjob.sh
      ;;
    sqlrunner)
      echo_info "Starting in 'sqlrunner' mode"
      /bin/bash $CRUNCHY_DIR/bin/sqlrunner/start.sh
      ;;
    *)
